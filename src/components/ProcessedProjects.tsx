...
// Helper to generate the correct link for a project
function getProjectHref(project: ProcessedProject): string {
  // Local agent wikis → /wiki/[id] route
  if (project.localWiki && project.wikiId) {
    return `/wiki/${project.wikiId}`;
  }
  // Backend-served wikis → /[owner]/[repo] route
  return `/${project.owner}/${project.repo}?type=${project.repo_type}&language=${project.language}&comprehensive=${project.comprehensive}${project.branch ? `&branch=${project.branch}` : ''}`;
}
