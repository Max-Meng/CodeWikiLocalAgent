...
const DEMO_SEQUENCE_CHART = `sequenceDiagram
  ...
  User->>CodeWiki: Provide repository path
  CodeWiki->>Repository: Analyze code structure
  Repository-->>CodeWiki: Return code analysis
  CodeWiki->>CodeWiki: Generate documentation
  CodeWiki-->>User: Display wiki with diagrams

  %% Add a note to make text more visible
  Note over User,Repository: CodeWiki supports sequence diagrams for visualizing interactions`;
            ...
            <>
              ...
              <div className="w-full max-w-2xl mb-10 bg-[var(--accent-primary)]/5 border border-[var(--accent-primary)]/20 rounded p-5">
                ...
                <p className="text-sm text-[var(--foreground)] mb-3">
                  Use the VS Code Copilot Agent to generate wikis, or place wiki JSON files in the public/wikis/ folder:
                </p>
                <div className="grid grid-cols-1 gap-3 text-xs text-[var(--muted)]">
                  <div className="bg-[var(--background)]/70 p-3 rounded border border-[var(--border-color)] font-mono overflow-x-hidden whitespace-nowrap">
                    node scripts/validate-wiki-data.js &lt;path-to-wiki.json&gt; --name &lt;wiki-id&gt; --fix
                  </div>
                </div>
              </div>
